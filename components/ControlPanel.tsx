  ...
  return (
    <div className="w-full h-16 md:h-20 bg-[#09090b]/95 backdrop-blur-xl border-b border-glass-border flex items-center justify-between px-4 md:px-6 z-50 sticky top-0 shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
      ...
      <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 hidden sm:flex">
        <div className="flex items-center gap-4 bg-black/40 px-5 py-2.5 rounded-full border border-white/5 backdrop-blur-md shadow-inner">
      ...
      <div className="flex items-center gap-2 md:gap-3 justify-end w-auto md:w-[240px]">
        ...
        <div className="flex items-center gap-2">
            ...
            <button 
                ...
                className={`group relative h-10 md:h-11 px-3 md:px-4 flex items-center justify-center gap-2 rounded-xl border transition-all duration-200 ${
                    authState.twitch 
                    ? 'bg-twitch/10 border-twitch text-twitch shadow-[0_0_15px_rgba(145,70,255,0.2)]' 
                    : 'bg-white/5 border-transparent text-gray-400 hover:bg-white/10'
                }`}
            >
                <TwitchLogo className="w-5 h-5 md:w-6 md:h-6" />
                <span className={`hidden md:block text-xs font-bold ${authState.twitch ? 'text-twitch' : 'text-gray-400 group-hover:text-gray-300'}`}>
            ...
            >
                <KickLogo className="w-5 h-5 md:w-6 md:h-6" />
                 <span className={`hidden md:block text-xs font-bold ${authState.kick ? 'text-kick' : 'text-gray-400 group-hover:text-gray-300'}`}>
                    {authState.kick ? 'ON' : 'OFF'}
                </span>
            </button>
        </div>

        <div className="h-6 md:h-8 w-px bg-white/10 mx-1 md:mx-2"></div>
        ...
        <div className="flex items-center gap-2">
            ...
            className="w-10 h-10 md:w-11 md:h-11 rounded-xl flex items-center justify-center bg-white/5 hover:bg-white/10 text-white transition-colors disabled:opacity-30 border border-white/5 active:scale-95"
            title="IA Analysis"
            >
                {isAnalyzing ? (
            ...
            className="w-10 h-10 md:w-11 md:h-11 rounded-xl flex items-center justify-center bg-dark hover:bg-white/5 text-gray-400 hover:text-white transition-colors border border-glass-border shadow-sm active:scale-95"
            title="Configurações"
            >
